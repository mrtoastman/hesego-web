...
class Loader {
	...
	public function dashboard_widget_display() {
		...
		<div>
			...
			<a href="<?php echo esc_url( admin_url( 'options-general.php?page=suretriggers' ) ); ?>" class="button button-primary"> <?php esc_html_e( 'Get Started', 'suretriggers' ); ?> </a>
		</div>
		<?php
	}
	...
	public function display_notice() {
		...
		<div class="notice notice-success" style="padding-bottom: 15px;">
			...
			<a href="<?php echo esc_url( admin_url( 'options-general.php?page=suretriggers' ) ); ?>" class="button button-primary"> <?php esc_html_e( 'Get Started With SureTriggers', 'suretriggers' ); ?> </a>
	...
	public function add_settings_link( array $links ) {
		$url           = get_admin_url() . 'admin.php?page=suretriggers';
		$settings_link = '<a href="' . $url . '">' . __( 'Settings', 'suretriggers' ) . '</a>';
		$links[]       = $settings_link;
		return $links;
	}
	...
	public function define_constants() {
		...
		define( 'SURE_TRIGGERS_VER', '1.0.9' );
		define( 'SURE_TRIGGERS_DB_VER', '1.0.9' );
	...
	private function get_localized_array() {
		...
		$data = [
			'siteContent'         => [
				...
				'resetUrl'     => base64_encode( wp_nonce_url( admin_url( 'options-general.php?st-reset=true' ), 'st-reset-action' ) ),
			],
			'user'                => [
	...
	public function reset_plugin() {
		...
		if ( $nonce && $is_reset && current_user_can( 'manage_options' ) && wp_verify_nonce( $nonce, 'st-reset-action' ) ) {
			...
			wp_safe_redirect( admin_url( 'options-general.php?page=suretriggers' ) );
			exit();
		}
	}
